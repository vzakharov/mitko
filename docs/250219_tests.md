...
- **Telegram handlers**: aiogram's built-in `MockedBot`
...
- [ ] `tests/handlers/test_start_command.py`
  - New user: greeting message, User/Chat created, state=`onboarding`
  - Existing user with profile: reset warning with confirmation keyboard
- [ ] `tests/handlers/test_match_acceptance.py` — **CRITICAL**
  - User A accepts first: status → `a_accepted`
  - User B accepts first: status → `b_accepted`
  - Second user accepts: status → `connected`, both notified
...
## Current State (19 tests, all passing)
