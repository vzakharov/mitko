...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('structured_data', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    op.add_column('users', sa.Column('embedding', Vector(1536), nullable=True))
    op.add_column('users', sa.Column('is_complete', sa.Boolean(), server_default='false', nullable=False))
    ...
    op.create_foreign_key('matches_user_a_id_fkey', 'matches', 'users', ['user_a_id'], ['telegram_id'])
    op.create_foreign_key('matches_user_b_id_fkey', 'matches', 'users', ['user_b_id'], ['telegram_id'])

    # Add index on embedding for vector search
    op.create_index('ix_users_embedding', 'users', ['embedding'], postgresql_using='ivfflat', postgresql_ops={'embedding': 'vector_cosine_ops'})
...
def downgrade() -> None:
    ...
    op.create_index('ix_profiles_embedding', 'profiles', ['embedding'], postgresql_using='ivfflat', postgresql_ops={'embedding': 'vector_cosine_ops'})
    ...
    op.add_column('matches', sa.Column('profile_a_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('matches', sa.Column('profile_b_id', postgresql.UUID(as_uuid=True), nullable=True))
    ...
    op.create_foreign_key('matches_profile_a_id_fkey', 'matches', 'profiles', ['profile_a_id'], ['id'])
    op.create_foreign_key('matches_profile_b_id_fkey', 'matches', 'profiles', ['profile_b_id'], ['id'])
